...
- OR (**Generator-Source Change**) the diff touches the REST Builder generator under `modules/util/portal-tools-rest-builder/`, such as a template or the generator Java. A generator change alters output for every module, so the full regen below catches modules the diff never touched.
